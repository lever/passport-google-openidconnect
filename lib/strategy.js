    ...
    this.configure(null, function(err, config) {
      ...
      oauth2.getOAuthAccessToken(code, { grant_type: 'authorization_code', redirect_uri: callbackURL }, function(err, accessToken, refreshToken, params) {
        ...
        if ( log_verbose )
        {
          ...
          if (params) { console.log('IT: ' + params.id_token); }
          console.log('----');
        }
        ...
        req.googleAuthParams.refreshToken = refreshToken;
        req.googleAuthParams.accessToken = accessToken;
        req.googleAuthParams.jwtClaims = jwtClaims;
        ...
        self._shouldLoadUserProfile(iss, sub, jwtClaims, function(err, load) {
          if (err) { return self.error(err); };
          ...
          if (load) {
            ...
            if ( log_verbose ) {
              console.log('Calling UserInfo url', userInfoURL);
            }
 ...
 * @api private
 */
Strategy.prototype._shouldLoadUserProfile = function(issuer, subject, jwt, done) {
  if (typeof this._skipUserProfile == 'function' && this._skipUserProfile.length > 1) {
    // async
    this._skipUserProfile(issuer, subject, jwt, function(err, skip) {
      if (err) { return done(err); }
      if (!skip) { return done(null, true); }
      return done(null, false);
    });
  } else {
    var skip = (typeof this._skipUserProfile == 'function') ? this._skipUserProfile(issuer, subject, jwt) : this._skipUserProfile;
    if (!skip) { return done(null, true); }
    return done(null, false);
  }
}
